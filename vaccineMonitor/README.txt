...
> src
    ...
    > tools
    - main.c
> tests (περιέχει virusesFile, countriesFile, inputFiles*)
- testFile.sh 
- Makefile
...
bloomSize: αριθμός που δηλώνει τα bytes του bloom filter.
...
max level = log_(2)(7.5M) = 32, αλλά τα αρχεία θα είναι αρκετά μικρότερου μεγέθους, οπότε έβαλα max level = 7.
...
Γίνεται διαχείριση των bits, οι hash functions έχει δοθεί και το Κ έχει οριστεί 16. Αναλυτικά σχόλια στον κώδικα εξηγούν τις λειτουργίες.
...
Στον φάκελο tests θα βρείτε ήδη αρχεία που έχουν δημιουργηθεί από το testFile.sh με το εξής format:
inputFiles50K_1.txt σημαίνει ότι έχουμε 50Κ γραμμές και το duplicatesAllowed = 1.
